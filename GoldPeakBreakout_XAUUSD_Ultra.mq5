//+------------------------------------------------------------------+
//|                              GoldPeakBreakout_XAUUSD_Ultra.mq5   |
//|  v4.0 ULTRA PROFIT: TP/SL dinamici ATR, Trail aggressivo,        |
//|       Sessione estesa, Filtri ottimizzati, Pyramiding opzionale  |
//+------------------------------------------------------------------+
#property copyright "User - Ultra Profit Version"
#property version   "4.00"
#property description "GoldPeak v4 ULTRA: ATR-based exits, aggressive trail, extended session"
...
input long     InpMagic            = 96008802;
input double   InpLots             = 0.10;
input string   InpTradeComment     = "GoldPeakUltra";

input string   InpSignal         = "=== Segnale Donchian (TF segnale) ===";
input ENUM_TIMEFRAMES InpSignalTf = PERIOD_M5;
input int      InpDonchianPeriod   = 18;           // Ottimizzato: leggermente più reattivo
input double   InpMinRangeUSD      = 2.5;          // Ridotto per più segnali
input double   InpEntryBufferPct   = 0.15;         // Percentuale ATR invece di fisso
input double   InpMinBreakoutAtrMult = 0.5;        // Dinamico basato su ATR
...
input double   InpAdxMin           = 18.0;         // RIDOTTO: cattura più trend

input bool     InpUseDiFilter      = true;
input double   InpDiMinSeparation  = 3.0;          // Leggermente ridotto

input bool     InpRequireMomentumBar = true;
input double   InpMinBodyRangePct = 0.35;          // Leggermente ridotto

input double   InpMinSignalRangeAtrMult = 0.60;    // Ridotto
input bool     InpUseRsiFilter    = false;         // DISATTIVATO DI DEFAULT per più trade
input int      InpRsiPeriod       = 14;
input double   InpRsiLongMin      = 50.0;          // Più permissivo
input double   InpRsiLongMax      = 75.0;
input double   InpRsiShortMax     = 50.0;
input double   InpRsiShortMin     = 25.0;

input string   InpExit             = "=== Uscite DINAMICHE (ATR Multiplier) ===";
input bool     InpUseDynamicTPSL  = true;          // NUOVO: TP/SL basati su ATR
input double   InpTpAtrMult       = 3.5;           // TP = 3.5x ATR (trend following)
input double   InpSlAtrMult       = 1.5;           // SL = 1.5x ATR (risk contenuto)
input double   InpTpFixedUSD      = 0.0;           // Se > 0, usa TP fisso invece di ATR
input double   InpSlFixedUSD      = 0.0;           // Se > 0, usa SL fisso invece di ATR
input int      InpMaxHoldBars      = 48;           // Esteso per trend lunghi

input string   InpTrail          = "=== Trail / BE AGGRESSIVO ===";
input bool     InpUseTrail        = true;          // ATTIVATO DI DEFAULT
input double   InpTrailAtrMult    = 1.2;           // Trail = 1.2x ATR
input double   InpTrailStartAtrMult = 1.0;         // Attiva trail dopo 1x ATR profitto
input bool     InpTrailOnNewBarOnly = false;

input bool     InpUseBreakEven     = true;
input double   InpBreakEvenAfterAtrMult = 0.8;     // BE dopo 0.8x ATR (più precoce)
input double   InpBreakEvenOffsetUSD = 0.3;

input string   InpFilters         = "=== Spread / ATR ===";
input int      InpMaxSpreadPoints = 450;           // Leggermente aumentato
input bool     InpUseAtrFilter    = true;
input int      InpAtrPeriod       = 14;
input double   InpMinAtrPoints    = 35;            // Ridotto

input string   InpFreq            = "=== Frequenza (barre = TF) ===";
input int      InpMaxTradesPerDay = 0;
input int      InpMinBarsBetweenTrades = 1;        // Ridotto per più frequenza
input bool     InpUseSessionFilter = true;
input int      InpSessionStartHour = 8;            // ESTESO: Londra open
input int      InpSessionEndHour  = 22;            // ESTESO: NY pomeriggio
...
input int      InpTesterMinTrades  = 15;
...
int OnInit()
  {
   ...
   if(InpUseDynamicTPSL && (InpTpAtrMult <= 0.0 || InpSlAtrMult <= 0.0))
     {
      Print("ATR multiplier per TP/SL devono essere > 0");
      return INIT_PARAMETERS_INCORRECT;
     }
...
void OnTick()
  {
   ...
   // Entry buffer dinamico basato su ATR
   const double buf = InpEntryBufferPct * g_currentATR;
   const double thrBuy = upper + buf;
   const double thrSell = lower - buf;
   const double c1 = iClose(_Symbol, InpSignalTf, 1);

   // Breakout threshold dinamico
   const double extra = MathMax(0.0, InpMinBreakoutAtrMult * g_currentATR);
...
void TryOpen(const ENUM_POSITION_TYPE type)
  {
   ...
   if(InpUseDynamicTPSL && g_currentATR > 0.0)
     {
      // TP/SL basati su ATR
      const double slDist = InpSlAtrMult * g_currentATR;
      const double tpDist = InpTpAtrMult * g_currentATR;
     ...
     {
      // Usa valori fissi se specificati, altrimenti default
      const double slUSD = (InpSlFixedUSD > 0.0) ? InpSlFixedUSD : 3.5;
      const double tpUSD = (InpTpFixedUSD > 0.0) ? InpTpFixedUSD : 10.0;
...
void ManagePosition(const ulong ticket, const int direction, const bool newBar)
  {
   ...
   // Break-even più precoce
   if(InpUseBreakEven)
     {
      const double profitDist = (direction == 1) ? (bid - openPrice) : (openPrice - ask);
      const double beThreshold = InpBreakEvenAfterAtrMult * g_currentATR;
   ...
   const double trailStart = InpTrailStartAtrMult * g_currentATR;
   
   if(InpTrailStartAtrMult > 0.0 && profitForTrail < trailStart)
      return;

   const double minD = StopsMinDistance();
   double newSL = sl;
   const double trailDist = InpTrailAtrMult * g_currentATR;

   if(direction == 1)
     {
      const double trail = NormalizePrice(bid - trailDist);
      if(trail > sl && trail < bid - minD)
         newSL = trail;
     }
   else
     {
      double trail = NormalizePrice(ask + trailDist);
      if(sl == 0.0)
        {
         if(trail > ask + minD)
            newSL = trail;
